...
class HAN_MODEL(nn.Module):
    def __init__(self, embedding_layer):
        ...
        # ablation
        self.linear_abaltion = nn.Linear(768, 2)
...
class Model(nn.Module):
    ...
    def forward(self, inputs_ids, attn_masks, manual_features,
        ...
        # 希望专家特征也能对句子进行影响
        han_logits, line_padding_mask = self.han_locator(line_ids, manual_features)
        ...
        if labels is not None:
            ...
            return loss, torch.softmax(logits,dim=1)[:, 1].unsqueeze(1), last_layer_attn_weights, torch.softmax(han_logits, dim=-1)[:, :, 1], line_padding_mask # shape: (bs, line_nums:256)
        else:
            # return torch.sigmoid(logits)[:, 1].unsqueeze(1)
            return torch.softmax(logits, dim=1)[:, 1].unsqueeze(1) , line_padding_mask
