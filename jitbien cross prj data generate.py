...
#%%
train_buggy_commit_lines_df = pd.read_pickle('./jitbien/train_buggy_commit_lines_df.pkl')
print('train data nums：', len(train_buggy_commit_lines_df))

valid_buggy_commit_lines_df = pd.read_pickle('./jitbien/valid_buggy_commit_lines_df.pkl')
print('valid data nums：', len(valid_buggy_commit_lines_df))

test_buggy_commit_lines_df = pd.read_pickle('./jitbien/test_buggy_commit_lines_df.pkl')
...
for prj in tqdm(project_list):
    print('-'*20)
    output_dir = './cross_prj_data/jitbien/'+prj+'/'
    ...
    # 数据保存
    path = './cross_prj_data/jitbien/'+prj+'/changes_train.pkl'
    output = open(path, 'wb')
    ...
    # 数据保存
    path = './cross_prj_data/jitbien/'+prj+'/changes_valid.pkl'
    output = open(path, 'wb')
    ...
    # 数据保存
    path = './cross_prj_data/jitbien/'+prj+'/changes_test.pkl'
    output = open(path, 'wb')
